    ...
    <div class="d-flex justify-content-between">
        ...
        <div id="admin-wrapper">
            ...
            <section id="admin-content" class="p-3">
                <h3 class="mb-4 text-center">Chỉnh sửa thông tin công việc</h3>
                <p class = "text-center text-danger">${message_edit }</p>
                <form method="post" action="<%=request.getContextPath()%>/task/edit">
                    <div class="row">
                        <div class="col-md-6 m-auto">
                        <input type="hidden" name="id" value="${taskOld.id }"/>
                            <div class="form-group">
                                <label>Tên công việc</label>
                                <input type="text" name="taskname" value="${taskOld.name }" class="form-control" placeholder="name" />
                            </div>
                            <div class="form-group">
                                <label>Mô tả</label>
                                <input type="text" name="description" value="${taskOld.description }" class="form-control" placeholder="description" />
                            </div>
                              <div class="form-group">
                                <label>Ngày bắt đầu</label>
                                <input type="text" name="startdate" value="${taskOld.startDate }" class="form-control" placeholder="yyyy-mm-dd" />
                            </div>
                              <div class="form-group">
                                <label>Ngày kết thúc</label>
                                <input type="text" name="enddate" value="${taskOld.endDate }" class="form-control" placeholder="yyyy-mm-dd" />
                            </div>
                            <div class="form-group">
                               <label>Người thực hiện</label> 
                                <select class="form-control" name="userid">
									<c:forEach items="${listUser }" var="item">
										<option value="${item.id }" <c:if test="${item.id == taskOld.userId}"> selected ="selected"</c:if>>${item.fullName }</option>
									</c:forEach>
								</select>
							</div>
                            <div class="form-group">
                                <label>Mã dự án</label>
                                <input type="text" name="project_id" value="${taskOld.projectId }" class="form-control" placeholder="project id" />
                            </div>
                            <div class="form-group">
                              <label>Trạng thái</label> 
                                <select class="form-control" name="statusid">
									<c:forEach items="${listStatus }" var="item">
										<option value="${item.id }" <c:if test="${item.id == taskOld.statusId}"> selected ="selected"</c:if>>${item.name }</option>
									</c:forEach>
								</select>
							</div>
                            <div class="form-group">
