    ...
    <div class="d-flex justify-content-between">
        ...
        <div id="admin-wrapper">
            ...
            <section id="admin-content" class="p-3">
                <h3 class="mb-4">Sửa công việc</h3>
                    ...
                    <div class="row">
                        ...
                        <input type="hidden" name="id" value="${taskDto.id }">
                        	<div class="form-group">
                                ...
                                <input type="text" name="name" class="form-control" value ="${taskDto.name }" />
                            </div>
                            <div class="form-group">
                                <label>Mô tả công việc</label>
                                <input type="text" name="description" class="form-control"  value ="${taskDto.description }" />
                            </div>
                            <div class="form-group">
                                <label>Ngày bắt đầu</label>
                                <input type="date" name="startdate" class="form-control"  value ="${taskDto.startDate }" />
                            </div>
                            <div class="form-group">
                                <label>Ngày kết thúc</label>
                                <input type="date" name="enddate" class="form-control"  value ="${taskDto.endDate }" />
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="statusid">
									<c:forEach items="${listStatus }" var="item">
										<option value="${item.id }" <c:if test="${item.id == taskDto.statusId}"> selected ="selected"</c:if>>${item.name }</option>
									</c:forEach>
								</select>
							</div>
                            <div class="form-group">
                                ...
                                <select class="form-control" name="userid">
									<c:forEach items="${listUser }" var="item">
										<option value="${item.id }" <c:if test="${item.id == taskDto.userId}"> selected ="selected"</c:if>>${item.fullName }</option>
									</c:forEach>
								</select>
							</div>
                            <div class="form-group">
