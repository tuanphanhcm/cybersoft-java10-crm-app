			...
			<tbody>
				<c:forEach var="item" items="${PROJECTS}">
					<tr>
						...
						<td>${item.endDate}</td>
						<td><a href="#" data-toggle="modal" data-target="#myModal">Xem danh sách</a>
							<div class="modal" id="myModal">
								<div class="modal-dialog">
									<div class="modal-content">
										<!-- Modal Header -->
										<div class="modal-header">
											<h4 class="modal-title">Dự án ${item.name}</h4>
											<button type="button" class="close" data-dismiss="modal">&times;</button>
										</div>
										<!-- Modal body -->
										<div class="modal-body">
												...
												<tbody>
													<c:forEach var="user" items="${item.users}">
														<tr>
															<td>${user.id}</td>
															<td>${user.fullName}</td>
															<td><a href="<c:url value='/prooject/deleteUser?userId=${user.id}&projectId=${item.id}'/>">
															Xóa						
															</a></td>
														</tr>
													</c:forEach>
